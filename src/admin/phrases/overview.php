        ...
        <div>
          <div class="flex h-80 flex-col items-center justify-center rounded-lg border border-neutral-700 p-6">
            <div class="mb-8 flex max-w-md flex-col gap-2 text-center">
              ...
              <span class="text-sm font-normal text-neutral-400"><?php echo htmlspecialchars($trans['You have no phrases created yet.']); ?></span>
            </div>
          </div>
        </div>
      <?php else : ?>
        <table class="min-w-full border-separate border-spacing-0 border-none text-left">
          <thead class="h-8 rounded-md bg-neutral-800">
            <tr>
              <th class="h-8 w-[100px] border-b border-t border-neutral-700 px-3 text-xs font-semibold text-neutral-400 first:rounded-l-md first:border-l last:rounded-r-md last:border-r">
                <?php echo htmlspecialchars($trans['admin_phrases_phrase_id']); ?>
              </th>
              <th class="h-8 w-[250px] border-b border-t border-neutral-700 px-3 text-xs font-semibold text-neutral-400 first:rounded-l-md first:border-l last:rounded-r-md last:border-r">
                <?php echo htmlspecialchars($trans['admin_phrases_phrase_title']); ?>
              </th>
              <th class="h-8 w-[250px] border-b border-t border-neutral-700 px-3 text-left text-xs font-semibold text-neutral-400 first:rounded-l-md first:border-l last:rounded-r-md last:border-r">
                <?php echo htmlspecialchars($trans['admin_phrases_phrase_content']); ?>
              </th>
              <th class="h-8 w-[100px] border-b border-t border-neutral-700 px-3 text-right text-xs font-semibold text-neutral-400 first:rounded-l-md first:border-l last:rounded-r-md last:border-r">
              ...
              <th class="h-8 w-[200px] border-b border-t border-neutral-700 px-3 text-right text-xs font-semibold text-neutral-400 first:rounded-l-md first:border-l last:rounded-r-md last:border-r">
                <?php echo htmlspecialchars($trans['admin_phrases_phrase_creation_date']); ?>
              </th>
              <th class="h-8 border-b border-t border-neutral-700 px-3 text-xs font-semibold text-neutral-400 first:rounded-l-md first:border-l last:rounded-r-md last:border-r">
              </th>
            </tr>
          </thead>
          <tbody>
                ...
                <td class="h-10 truncate border-b border-neutral-700 px-3 text-center text-sm">
                  ...
                  <div id="dropdownMenu_<?php echo $phrase['id']; ?>" class="hidden origin-top-right mt-1 absolute right-[0.75rem] md:right-[0.5rem] z-50 min-w-[8rem] overflow-hidden rounded-md border text-popover-foreground shadow-md data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 border-neutral-700 bg-neutral-900 p-1">
                    <div class="flex flex-col gap-1 my-1">
                      <a href="../../phrase/view.php?id=<?php echo $phrase['id']; ?>" role="menuitem" class="flex items-center gap-2 rounded-sm border border-transparent px-1 text-neutral-400 hover:text-neutral-100 hover:bg-neutral-800 focus-visible:border-neutral-600 focus-visible:bg-neutral-700 focus-visible:text-neutral-100 focus-visible:outline-none">
                        <i data-lucide="eye" class="size-4"></i>
                        <span class="flex items-center gap-1 text-sm font-medium"><?php echo htmlspecialchars($trans['admin_phrases_view_phrase']); ?></span>
                      </a>
                      <a href="./edit.php?id=<?php echo $phrase['id']; ?>" role="menuitem" class="flex items-center gap-2 rounded-sm border border-transparent px-1 text-neutral-400 hover:text-neutral-100 hover:bg-neutral-800 focus-visible:border-neutral-600 focus-visible:bg-neutral-700 focus-visible:text-neutral-100 focus-visible:outline-none">
                        <i data-lucide="pencil" class="size-4"></i>
                        <span class="flex items-center gap-1 text-sm font-medium"><?php echo htmlspecialchars($trans['admin_phrases_edit_phrase']); ?></span>
                      </a>
                      <?php if ($phrase['visibility'] == '0') : ?>
                        <form method="POST">
                          ...
                          <button type="submit" name="make_public" class="flex items-center gap-2 rounded-sm border border-transparent px-1 text-neutral-400 hover:text-neutral-100 hover:bg-neutral-800 focus-visible:border-neutral-600 focus-visible:bg-neutral-700 focus-visible:text-neutral-100 focus-visible:outline-none">
                            <i data-lucide="eye" class="size-4"></i>
                            <span class="flex items-center gap-1 text-sm font-medium"><?php echo htmlspecialchars($trans['admin_phrases_show_phrase']); ?></span>
                          </button>
                        </form>
                      <?php elseif ($phrase['visibility'] == '1') : ?>
                        <form method="POST">
                          ...
                          <button type="submit" name="make_private" class="flex items-center gap-2 rounded-sm border border-transparent px-1 pl-auto text-neutral-400 hover:text-neutral-100 hover:bg-neutral-800 focus-visible:border-neutral-600 focus-visible:bg-neutral-700 focus-visible:text-neutral-100 focus-visible:outline-none">
                            ...
                            <span class="flex items-center gap-1 text-sm font-medium mr-3"><?php echo htmlspecialchars($trans['admin_phrases_hide_phrase']); ?></span>
                          </button>
                        </form>
                      <?php endif; ?>
                      <form method="POST" onsubmit="return confirm('<?php echo htmlspecialchars($trans['admin_phrases_delete_phrase_confirmation']); ?>')">
                        <input type="hidden" name="phrase_id" value="<?php echo $phrase['id']; ?>">
