...
<head>
  ...
  <title><?php echo htmlspecialchars($trans['admin_page_title']); ?> ·
    <?php echo htmlspecialchars($trans['admin_users_page_title']); ?></title>
        ...
        <div class="flex h-80 flex-col items-center justify-center rounded-lg border border-neutral-700 p-6">
          <div class="mb-8 flex max-w-md flex-col gap-2 text-center">
            <h2 class="text-xl font-bold tracking-[-0.16px] text-neutral-100">
              <?php echo htmlspecialchars($trans['admin_users_no_users_found']); ?></h2>
            <span
              class="text-sm font-normal text-neutral-400"><?php echo htmlspecialchars($trans['admin_users_no_users_explanation']); ?></span>
          </div>
        </div>
      <?php else: ?>
        <table class="min-w-full border-separate border-spacing-0 border-none text-left">
          <thead class="h-8 rounded-md bg-neutral-800">
            <tr>
              <th
                class="h-8 w-[150px] border-b border-t border-neutral-700 px-3 text-xs font-semibold text-neutral-400 first:rounded-l-md first:border-l last:rounded-r-md last:border-r">
                <?php echo htmlspecialchars($trans['admin_users_user_id']); ?></th>
              <th class="h-8 w-[250px] border-b border-t border-neutral-700 px-3 text-xs font-semibold text-neutral-400">
                <?php echo htmlspecialchars($trans['admin_users_user_name']); ?></th>
              <th class="h-8 w-[300px] border-b border-t border-neutral-700 px-3 text-xs font-semibold text-neutral-400">
                <?php echo htmlspecialchars($trans['admin_users_user_email']); ?></th>
              <th class="h-8 w-[250px] border-b border-t border-neutral-700 px-3 text-xs font-semibold text-neutral-400">
                <?php echo htmlspecialchars($trans['admin_users_user_email_confirmed']); ?></th>
              <th class="h-8 w-[200px] border-b border-t border-neutral-700 px-3 text-xs font-semibold text-neutral-400">
                <?php echo htmlspecialchars($trans['admin_users_user_signup_time']); ?></th>
              <th class="h-8 border-b border-t border-neutral-700 px-3 text-xs font-semibold text-neutral-400">
                <?php echo htmlspecialchars($trans['admin_users_user_status']); ?></th>
              <th
          ...
          <tbody>
            <?php foreach ($users as $user): ?>
              <tr>
                ...
                <td class="h-10 truncate border-b border-neutral-700 px-3 py-3 text-sm">
                  <?php echo $user['confirmed'] ? $trans['admin_user_view_confirmed'] : $trans['admin_user_view_not_confirmed']; ?>
                </td>
                <td class="h-10 truncate border-b border-neutral-700 px-3 py-3 text-sm"><?php echo $user['signup_time']; ?>
                </td>
                <td class="h-10 truncate border-b border-neutral-700 px-3 py-3 text-center text-sm">
                  <?php echo $user['status'] ? $trans['admin_user_view_status_blocked'] : $trans['admin_user_view_status_active']; ?>
                </td>
                <td class="h-10 truncate border-b border-neutral-700 px-3 py-3 text-center text-sm">
                  <button id="dropdownButton_<?php echo $user['id']; ?>" type="button"
                  ...
                  <div id="dropdownMenu_<?php echo $user['id']; ?>"
                    ...
                    <div class="flex flex-col gap-1 my-1">
                      <a href="./view.php?id=<?php echo $user['id']; ?>" role="menuitem"
                        class="w-ful flex items-center gap-2 rounded-sm border border-transparent px-1 text-neutral-400 hover:text-neutral-100 hover:bg-neutral-800 focus-visible:border-neutral-600 focus-visible:bg-neutral-700 focus-visible:text-neutral-100 focus-visible:outline-none">
                        <i data-lucide="eye" class="size-4"></i>
                        <span
                          class="flex items-center gap-1 text-sm font-medium"><?php echo htmlspecialchars($trans['admin_users_view_user']); ?></span>
                      </a>
                      <a href="./edit.php?id=<?php echo $user['id']; ?>" role="menuitem"
                        ...
                        <span
                          class="flex items-center gap-1 text-sm font-medium"><?php echo htmlspecialchars($trans['admin_users_edit_user']); ?></span>
                      </a>
                      <?php if ($user['status'] == 1): ?>
                        <form method="POST">
                          ...
                          <button type="submit" name="unblock_user"
                            ...
                            <span
                              class="flex items-center gap-1 text-sm font-medium"><?php echo htmlspecialchars($trans['admin_users_unblock_user']); ?></span>
                          </button>
                        </form>
                      <?php elseif ($user['status'] == 0): ?>
                        <form method="POST">
                          ...
                          <button type="submit" name="block_user"
                            ...
                            <span
                              class="flex items-center gap-1 text-sm font-medium"><?php echo htmlspecialchars($trans['admin_users_block_user']); ?></span>
                          </button>
                        </form>
                      <?php endif; ?>
                      <form method="POST"
                        onsubmit="return confirm('<?php echo htmlspecialchars($trans['admin_users_delete_user_confirmation']); ?>')">
                        <input type="hidden" name="user_id" value="<?php echo $user['id']; ?>">
                        <button type="submit" name="delete_user" role="menuitem"
